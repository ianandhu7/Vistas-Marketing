 ...
 * 
 */
package co.vistafoundation.vlearning.exam_preparation.service.impl;
...
@Service
public class ExamPreparationServiceImpl implements ExamPreparationService {
	...
	private static final Logger log = LoggerFactory.getLogger(ExamPreparationServiceImpl.class);
			...
			for (Object idQuestion : uniqueQuestionIdList) {
				...
				for (int i = 0; i < tempQuestionList.size(); i++) {
					...
					if (i == 0) {
						...
						if (questionData.get("questionType").equalsIgnoreCase("MultiSelect")
								|| questionData.get("questionType").equalsIgnoreCase("Objective")) {
							...
							quizQuestion.setQuestionType(questionData.get("questionType"));
